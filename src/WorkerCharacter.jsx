...
export default function WorkerCharacter({ variant:v, index, action, target }) {
  ...
  return <group name={`crew-${index}`} ref={root} position={v.home} scale={[1.18*v.width,1.18*v.height,1.18]}>
    <group ref={body}>
      ...
      <Part color="#d5c48f" position={[0,.69,.137]} scale={[.065,.05,.016]} />
      <Part name="utility-bag" color="#9b8055" position={[-.23,.66,-.02]} scale={[.12,.18,.14]} />
      ...
      <group ref={head} position={[0,1.4,0]}>
        ...
        <Part color="#814c3a" position={[0,-.091,.163]} scale={[.056,.014,.016]} />
      ...
      {[-1,1].map(side=>{const key=side<0?'left':'right';return <group key={key}>
        <group ref={ref(key+'Arm')} position={[side*.255,1.075,0]} rotation={[0,0,side*-.08]}>
          ...
          <Part color="#ddd1a4" position={[0,-.21,0]} scale={[.145,.05,.16]} />
